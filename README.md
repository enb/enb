...
C помощью методов `cache.putFile(key, content[, opts])` и `cache.getFile(key, sourceMtime[, opts])` можно сохранять в кэш результаты обработки каких-то файлов для дальнейшего их реиспользования.
Аргументы:
...
* `opts` - дополнительные опции
  * `nodePrefix` - флаг, нужно ли добавлять префикс ноды к ключу. Префикс позволяет избежать конфликтов при кэшировании одного и того же файла в разных нодах, но при этом снижает эффективность файлового кэша. По-умолчанию префикс добавляется.
