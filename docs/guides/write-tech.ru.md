...
Цель технологии — собирать [таргет](../terms.ru.md) в ноде. Например, технология `css` может собрать `index.css` в ноде `pages/index` на основе `css`-файлов по [уровням переопределения](https://ru.beta.bem.info/methodology/key-concepts/#Уровень-переопределения).
