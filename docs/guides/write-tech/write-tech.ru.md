...
В данном руководстве охвачены не все возможности `BuildFlow`. Полный перечень методов с описанием находится в JSDoc файла `build-flow.js`.

## Теория

Цель технологии — собирать [таргет](../../terms/terms.ru.md) в ноде. Например, технология `css` может собрать `index.css` в ноде `pages/index` на основе `css`-файлов по [уровням переопределения](https://ru.beta.bem.info/methodology/key-concepts/#Уровень-переопределения).
...
Технологии могут использовать результат выполнения других технологий. Например, список исходных `css`-файлов строится с помощью технологии `files`.
...
Каждый `use`-метод добавляет аргумент в `builder`. Тип и содержимое аргументов зависят от того, какой `use`-метод был использован.
...
В каждой технологии, сделанной с помощью `BuildFlow`, есть метод `buildFlow()`, который можно вызвать,
...
Чтобы вместе с суффиксами `css` дополнительно собирать еще и `light.css`, необходимо написать новую технологию, заимствуя функциональность старой:
