...
var parseXml = exports.parseXml = function(xml, cb) {
    try {
        new domjs.DomJS().parse('<root>' + xml + '</root>', function(err, dom) {
            if(err) return;
            cb(dom.children);
        });
    } catch(e) {
        parseXml(domjs.escape(xml), cb);
    }
};
...
var domToJs = exports.domToJs = function(nodes) {
    ...
    return code.length === 1 &&
        (code[0].charAt(0) === quoteChar || code[0].charAt(0) === singleQuoteChar ) ?
    ...
    switch(type) {
        ...
        case 'XML':
            ...
            var tag = item[0],
                ...
                prop = [],
                a;
            for(a in attrs) { prop.push([a, singleQuoteChar + attrs[a] + singleQuoteChar].join('=')); };
        ...
        case 'TEXT':
            // [ text ]
            return _raw ? item[0] : singleQuoteChar + jsQuote(item[0]) + singleQuoteChar;
        default:
