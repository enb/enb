...
var BuildFlow = inherit(/** @lends BuildFlow.prototype */ {
    ...
    useSourceListFilenames: function (targetOptionName, defaultTargetNames) {
        return this._copyAnd(function (buildFlow) {
            buildFlow._addUsage(
                new BuildFlowLinkToTargetList(targetOptionName, defaultTargetNames, BuildFlowLinkToTargetResult)
            );
        });
    },
    ...
    createTech: function () {
        ...
        var resultTechMethods = {
            ...
            __saveFunc: function(target, result) {
                var node = this.node;
                var tmpFiles = node._makePlatform._tmpFiles;
                var targetNamesToBuild = node._targetNamesToBuild;

                var isTargetToBuild = _.some(targetNamesToBuild, function(targetToBuild) {
                    return target === targetToBuild;
                });

                var targetPath = node.resolvePath(target);

                node.resolveTarget(target, {path: targetPath, content: result})

                if(tmpFiles || isTargetToBuild) {
                    return saveFunc(targetPath, result);
                }
            },
            __wrapFunc: wrapFunc,
            __preparer: preparer,
                        ...
                        if (_this._isRebuildRequired()) {
                            return Vow.when(_this._getBuildResult.apply(_this, results)).then(function (data) {
                                return Vow.when(_this._wrapBuildResult(data)).then(function (wrappedData) {
                                    ...
                                    return Vow.when(
                                        _this._saveBuildResult(_this._target, wrappedData)
                                    ).then(function () {
                                        _this._saveCache();
