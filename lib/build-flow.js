...
const BuildFlowLinkToTargetResult = inherit({
    ...
    requireTarget(tech, node) {
        ...
        return node.requireSources([tech[this._fieldName]])
            .spread(result => processTargetResult.call(this, tech, node, result));
    },
    /**
...
const BuildFlowLinkToDirList = inherit(BuildFlowLinkToTargetResult, {
    ...
    isCacheValid(tech, node, cache) {
        const targetName = tech[this._fieldName];
        const files = [].concat.apply([], tech[this._listName].map(dir => dir.files));
...
const BuildFlowLinkToTargetList = inherit({
    ...
    configureUsages(tech, node) {
        ...
        let i = 0;
        tech.getOption(this._targetOptionName, this._defaultTargetNames).forEach(targetName => {
     ...
     */
    saveCache(tech, node, cache) {
        tech[this._usagesFieldName].forEach(link => {
     ...
     */
    requireTarget(tech, node) {
        return Vow.all(tech[this._usagesFieldName].map(link => link.requireTarget(tech, node)));
    }
});
...
const BuildFlow = inherit(/** @lends BuildFlow.prototype */ {
    ...
    __constructor() {
        ...
        this._wrapFunc = data => data;
        this._saveFunc = (filename, result) => vowFs.write(filename, result, 'utf8');
        this._cacheValidator = () => false;
        this._cacheSaver = () => {};
    },
     ...
     */
    name(techName) {
        return this._copyAnd(buildFlow => {
    ...
    copy() {
        ...
        Object.keys(optionAliases).forEach(optionName => {
    ...
    justJoinFiles(wrapper) {
        return this._copyAnd(buildFlow => {
            buildFlow._buildFunc = function () {
                const _this = this;
                return Vow.all(Array.prototype.map.call(arguments, arg => {
                    if (typeof arg === 'string') {
                        return vowFs.read(arg, 'utf8').then(data => wrapper ? wrapper.call(_this, arg, data) : data);
    ...
    _removeUsage(targetOptionName) {
        this._usages = this._usages.filter(link => link.getTargetOptionName() !== targetOptionName);
    },
    _addDep(dep) {
    ...
    createTech() {
        ...
        const resultTechMethods = {
            ...
            _requireSources() {
                const _this = this;
                const node = this.node;
                return Vow.all(usages.map(usage => usage.requireTarget(_this, node)));
            },
            _isRebuildRequired() {
            ...
            _joinFiles(files, wrapper) {
                const _this = this;
                return Vow.all(files.map(fileInfo => {
                    return vowFs.read(fileInfo.fullname, 'utf8').then(data => {
                        return wrapper ? wrapper.call(_this, fileInfo.fullname, data) : data;
                    });
                })).then(results => results.join(EOL));
            },
            _joinFilesWithComments(files) {
                const node = this.node;
                return this._joinFiles(files, (filename, data) => {
            ...
            _preprocessTargetName(targetName) {
                const _this = this;
                return targetName.replace(/{([^}]+)}/g, (s, optName) => {
            ...
            build() {
                const _this = this;
                const node = this.node;
                return Vow.when(_this._prepare()).then(() => {
                    ...
                    return _this._requireSources().then(results => {
                        if (_this._isRebuildRequired()) {
                            return Vow.when(_this._getBuildResult.apply(_this, results)).then(data => {
                                return Vow.when(_this._wrapBuildResult(data)).then(wrappedData => {
