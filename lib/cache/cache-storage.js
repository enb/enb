 ...
 * @name CacheStorage
 */
const CacheStorage = inherit(/** @lends CacheStorage.prototype */ {
    /**
     *
     * @name CacheStorage
    ...
    saveAsync() {
        ...
        return new vow.Promise((resolve, reject) => {
            const prefixes = Object.keys(_this._data);
            const stream = fs.createWriteStream(_this._filename)
    ...
    invalidate(prefix, key) {
        const prefixObj = this._data[prefix];
        if (prefixObj) {
