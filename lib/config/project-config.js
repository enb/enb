...
module.exports = inherit( /** @lends ProjectConfig.prototype */ {
     ...
     * @returns {ProjectConfig}
     */
    node: function (nodePath, func) {
        nodePath = path.sep === '\/' ? nodePath.replace(/^\/+|\/+$/g, '') : nodePath.replace(/\\+$/g, '');
    ...
    nodes: function () {
        ...
        function toRelative(nodePath) {
            return nodePath.replace(root + path.sep, '');
        }
        while (!flat) {
            flat = true;
            result = [];
            for (var i = 0, l = input.length; i < l; i++) {
                ...
                if (typeof item === 'function') {
                    ...
                    if (~item.indexOf('*')) {
                        result = result.concat(dirGlob.globSync(path.join(root, item)).map(toRelative));
                    } else {
                        result.push(item);
                    }
                }
