...
module.exports = inherit(/** @lends MakePlatform.prototype */ {
    ...
    loadCache: function () {
        ...
        return this._isCacheValid() || this._cacheStorage.drop();
    },

    /**
     * Асинхронно загружает кэш из временной папки.
     * @see loadCache
     */
    loadCacheAsync: function () {
        var _this = this;
        this._cacheStorage.loadAsync()
            .then(function () {
                return _this._isCacheValid() || _this._cacheStorage.drop();
            });
    },

    /**
     * Проверяет загруженный кеш на свежесть, режим сборки и соответствие версии сборщика.
     * В случае, если обновился пакет enb, либо изменился режим сборки, либо изменились make-файлы — кэш невалиден.
     * @returns {Boolean} - Валиден ли кеш
     * @private
     */
    _isCacheValid: function () {
        ...
        // TODO: Put version to the file name.
