...
module.exports = inherit(/** @lends Node.prototype */ {
    ...
    createTmpFileForTarget(targetName) {
        ...
        function createTmpFilename() {
            var prefix = '_tmp_' + (new Date()).getTime() + Math.floor(Math.random() * 0x1000000000).toString(36) + '_';
    ...
    requireNodeSources(sourcesByNodes) {
        return Vow.all(Object.keys(sourcesByNodes).map(node => {
            var sources = sourcesByNodes[node];
    ...
    _registerTargets() {
        var _this = this;
        this._techs.forEach(function (tech) {
    ...
    _expandTargets(targets, defaultTargetList) {
        var allTargets = defaultTargetList && defaultTargetList.length === 0
