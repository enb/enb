...
module.exports = options => {
    return (req, res, next) => {
        if (req.method === 'GET' && req.url === '/') {
            const projectName = path.basename(options.dir);
            const makePlatform = new MakePlatform();

            makePlatform.init(options.dir, options.mode, options.config, { graph: true }).done(() => {
                if (options.cache) {
                    makePlatform.loadCache();
                }
                return makePlatform.buildTargets([]).then(() => {
                    const htmlPages = [];
                    const nodeConfigs = makePlatform.getProjectConfig().getNodeConfigs();

                    Object.keys(nodeConfigs).forEach(nodeName => {
                        const nodeConfig = nodeConfigs[nodeName];

                        nodeConfig.getTargets().forEach(target => {
                            if (/\.html$/.test(target)) {
                    ...
                    makePlatform.getBuildGraph().renderFragmentedPumlLinks().forEach(graphLink => {
                        html.push(
                            `<li><img src="${graphLink}" /></li>`
                        );
                    });

                    html.push('</ul>');
                    html.push('<h2>More info</h2>');
                    html.push('<div><a href="https://github.com/enb/enb">ENB Readme</a></div>');
                    html.push('</body>');
                    html.push('</html>');
